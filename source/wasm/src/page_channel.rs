...
pub fn build(pc: &mut ProcessingContext, m: &MinistateChannel) -> El {
    ...
    head_bar.back_unread.ref_own(|el_| nol_channel.then({
        let el_ = el_.weak();
        let eg = pc.eg();
        move |channel| {
            ...
            el_.ref_own(
                |el_| eg
                    .event(
                        |pc| link!(
                            (_pc = pc),
                            (unread_all = state().unread.clone(), unread_channel = channel.unread.clone()),
                            (),
                            (el_ = el_.weak()) {
                                let el_ = el_.upgrade()?;
                                let unread = *unread_all.borrow() - *unread_channel.borrow();
                                if unread > 0 {
